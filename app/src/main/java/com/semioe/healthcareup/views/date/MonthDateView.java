package com.semioe.healthcareup.views.date;
...
public class MonthDateView extends View {
	...
	private int mCircleColor = Color.parseColor("#d46576");
	...
	protected void onDraw(Canvas canvas) {
		...
		for(int day = 0;day < mMonthDays;day++){
			...
			if(dayString.equals(mSelDay+"")){
				...
				float circleX = (float) (startRecX +	endRecX)/2f;
				float circley = (float) (startRecY +	endRecY)/2f;
...
//				canvas.drawRect(startRecX, startRecY, endRecX, endRecY, mPaint);
			...
			if(tv_date != null){
				tv_date.setText(mSelYear + "-" + String.format("%02d", (mSelMonth + 1)));
			}
	...
	private void drawCircle(int row,int column,int day,Canvas canvas){
		if(daysHasThingList != null && daysHasThingList.size() >0){
			...
			canvas.drawText("记",circleX-mCircleRadius/2.5f, circley+mCircleRadius/4,mPaint);
			mPaint.setTextSize(mDaySize*mDisplayMetrics.scaledDensity);
		}
	}
	@Override
